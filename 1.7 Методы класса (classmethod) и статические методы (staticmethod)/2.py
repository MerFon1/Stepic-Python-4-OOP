...
class TextInput:
    CORRECT_NAME = "абвгдеёжзийклмнопрстуфхцчшщьыъэюяabcdefghijklmnopqrstuvwxyz0123456789 "
    ...
    def check_name(cls, name):
        r = [i.lower() in cls.CORRECT_NAME for i in name]
        if all(r) == True and 3 <= len(name) <= 50:
            return name
        raise ValueError("некорректное поле name")
