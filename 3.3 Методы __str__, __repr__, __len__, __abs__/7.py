...
class DeltaClock:
    ...
    def __str__(self):
        if self.clock1.get_time() - self.clock2.get_time()<0:
            ...
            self.r = time.strftime('%H: %M: %S', time.gmtime(self.clock1.get_time() - self.clock2.get_time()))
            return self.r

    def __len__(self):
        p = self.r.split(': ')
        out_l = Clock(int(p[0]),int(p[1]),int(p[2]))
        return out_l.get_time()


dt = DeltaClock(Clock(2, 45, 0), Clock(13, 15, 0))
print(dt)
len_dt = len(dt)
print(len_dt)